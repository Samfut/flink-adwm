...
def compare_adwm_perwm(path):
    ...
    if "CB" in path:
        ...
        df.to_csv(f"./WaitTime/{filename}.csv")
...
def get_drop_latency(wm, latency):
    ...
    if 'DIDI' in filename:
        df['com'] = [120000] * len(df.index)
    if 'CB' in filename:
        df['com'] = [1000] * len(df.index)  
    df.to_csv(f'./WindowWait/{filename}.csv')
